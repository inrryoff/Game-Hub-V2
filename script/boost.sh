...
FONTE_MODULO="$MODPATH/bins/rgb.c"
FONTE_TEMP="/data/data/com.termux/files/home/rgb.c"
...
# ============================================================
install_dp() {
    if [ -f "$CHECK_DP" ]; then return 0; fi

    if [ -d "/data/data/com.termux" ]; then
        local TERMUX_USER=$(stat -c '%U' /data/data/com.termux/files/usr)
        
        if ! command -v clang > /dev/null; then
            say "${YELLOW}🛠️ Instalando ferramentas...${NC}"
            su "$TERMUX_USER" -c "pkg update -y && pkg install clang ncurses-utils -y" > /dev/null 2>&1
        fi

        if [ -f "$FONTE_MODULO" ]; then
            say "${YELLOW}🚀 Preparando compilação...${NC}"
            cp "$FONTE_MODULO" "$FONTE_TEMP"
            chown "$TERMUX_USER:$TERMUX_USER" "$FONTE_TEMP"
            
            su "$TERMUX_USER" -c "clang -O3 '$FONTE_TEMP' -lm -o '$PREFIX/bin/rgb'"
            
            if [ -f "$PREFIX/bin/rgb" ]; then
                su "$TERMUX_USER" -c "chmod +x '$PREFIX/bin/rgb'"
                rm -f "$FONTE_TEMP"
                say "${GREEN}✅ Motor RGB compilado em /bin/rgb${NC}"
            else
                say "${RED}❌ Erro: O binário não foi gerado.${NC}"
                return 1
            fi
        else
            say "${RED}❌ Fonte não encontrado em: $FONTE_MODULO${NC}"
        fi
    fi
    touch "$CHECK_DP"
}
