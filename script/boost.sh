...
CACHE_FILE="$MODPATH/common/ren{derer_cache.cfg"
...
say() { echo -e "$1" >&3; }
say_rgb() { echo -e "$1" | neonx -L -m 2  -o 0 >&3; }
...
# ============================================================
install_dp() {
    if command -v neonx > /dev/null 2>&1; then
        return 0
    fi
    ...
    case $ARCH in
        arm64-v8a)
            NEONX_PATH="$MODPATH/bins/NeonX_arm64"
            ;;
        armeabi-v7a|armeabi|armv7l)
            NEONX_PATH="$MODPATH/bins/NeonX_arm32"
            ;;
        *)
    ...
    if [ -f "$NEONX_PATH/neonx" ]; then
        say "${CYAN}🚀 Instalando Motor de Renderização Nativo...${NC}"
        cp "$NEONX_PATH/neonx" "$PREFIX/bin/"
        chmod +x "$PREFIX/bin/neonx"
        
        if [ -x "$PREFIX/bin/neonx" ]; then
            say "${GREEN}✅ Motor NeonX (v2.0.2) instalado com sucesso!${NC}"
            sleep 2
        else
            say "${RED}❌ Erro crítico: Falha ao configurar o binário.${NC}"
            sleep 5
            return 1
        fi
    else
        say "${RED}❌ Erro: Binário não encontrado em $NEONX_PATH${NC}"
        say "${YELLOW}Verifique se o download do módulo foi concluído corretamente.${NC}"
        sleep 5
        return 1
    fi
}
...
_spinner_anim() {
    ...
    local cores=($(neonx --spin 2>/dev/null))
    ...
    while kill -0 "$pid" 2>/dev/null; do
        for frame in "${frames[@]}"; do
            ...
            printf "\r\e[K\e[%s[%s]\e[0m \e[33m%s\e[0m" "$cor" "$frame" "$msg"
            idx=$((idx + 1))
            sleep "$delay"
...
exterminar_apps() {
    ...
    LAUNCHER=$(cmd package resolve-activity -c android.intent.category.HOME -a android.intent.action.MAIN | grep -i 'packageName' | head -n 1 | cut -d '=' -f 2)
...
show_banner() {
    command clear
    say ""
    cat << "EOF" | neonx -d 1.5 -m 7 -f 0.5 -s 0.2 -o 0.3
╔════════════════════════════════════════════════════════════════════════════╗
║  ██████╗░█████╗░██████╗░███████╗      ██████╗░░█████╗░███╗░░░███╗███████╗  ║
║  ██╔═══╝██╔══██╗██╔══██╗██╔════╝      ██╔═══╝ ██╔══██╗████╗░████║██╔════╝  ║
║  ██║░░░░██║░░██║██████╔╝█████╗░░█████╗██║░░██╗███████║██╔████╔██║█████╗░░  ║
...
╚════════════════════════════════════════════════════════════════════════════╝
EOF
    say_rgb "                         ${_SCRIPT_NAME} v${_VERSION} by ${_AUTHOR}"
    say_rgb "  ══════════════════════════════════════════════════════════════════════════"
} 1>&3
